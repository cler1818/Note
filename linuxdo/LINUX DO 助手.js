...
// @version      6.7.10
// @description  日常维护/快速升级/日常挂机三种刷帖模式，点赞与刷帖交错、匀速铺满整个运行时长。面板显示信任等级、等级3进度(TL2+)或摘要统计(TL0/1)、LINUX DO Credit 积分。集成 AgentRouter 每日自动签到、AnyRouter 一键登录(含封禁检测)、Credit 后台自动登录，以及各站点「使用 LinuxDO 继续」按钮与 Connect 授权页「允许」的自动点击。
// @match        https://*/*
// @grant        GM_setValue
// @grant        GM_getValue
...
(function () {
    ...
    function gmJsonGet(k, fb) { try { const v = JSON.parse(gmGet(k, "null")); return v === null ? fb : v; } catch (_) { return fb; } }
    ...
    // ---- OAuth client_id 白名单（Q1=b：3个预置 + confirm 学习）----
    const OAUTH_PRESET = [AR.CLIENT_ID, ANY.CLIENT_ID, CREDIT.CLIENT_ID];
    const OAUTH_LEARNED_KEY = "ldh_oauth_allowed_v1";      // 用户 confirm 同意过的 client_id 数组
    const OAUTH_PENDING_KEY = "ldh_oauth_pending_v1";      // 后台标签遇到的未知 client_id，留待前台再问
    function learnedClients() { const a = gmJsonGet(OAUTH_LEARNED_KEY, []); return Array.isArray(a) ? a : []; }
    function clientAllowed(id) { if (!id) return false; return OAUTH_PRESET.indexOf(id) >= 0 || learnedClients().indexOf(id) >= 0; }
    function learnClient(id) { if (!id) return; const a = learnedClients(); if (a.indexOf(id) < 0) { a.push(id); gmSet(OAUTH_LEARNED_KEY, JSON.stringify(a)); } }
    function clearLearnedClients() { gmDel(OAUTH_LEARNED_KEY); gmDel(OAUTH_PENDING_KEY); }
    ...
    const KNOWN_HOSTS = ["agentrouter.org", "anyrouter.top", "credit.linux.do", "linux.do", "connect.linux.do"];
    function isKnownHost() { return KNOWN_HOSTS.indexOf(location.hostname) >= 0; }

    /* ============================================================
     * 通用站点分支（非四个已知站点）：只做「自动点登录按钮」（Q3=a / Q9=10秒）
     * 注意：这段必须在任何 GM 存储读写之前 return，
     *       否则每打开一个网页都会去读一次油猴存储，白白拖慢浏览。
     * ============================================================ */
    if (!isKnownHost()) {
        // 首页(/)不点，避免在任意网站首页误触；仅登录类路径
        const p = (location.pathname || "").toLowerCase();
        if (p !== "/" && p !== "" && /login|signin|sign-in|auth|oauth|sso/.test(p)) {
            waitAndClick(findLdLoginBtn, 10000);
        }
        return;
    }
    ...
    if (location.hostname === "connect.linux.do") {
        if (location.pathname === "/oauth2/authorize") {
            ...
            // Q5=a：后台标签的 confirm 会被浏览器吞掉 → 记下来，等切到前台再问
            const inBackground = (typeof document.visibilityState === "string" && document.visibilityState === "hidden");

            const doClickAllow = function () {
                const bg = document.visibilityState === "hidden";
                return waitAndClick(function () {
            ...
            const askAndMaybeAllow = function (id) {
                let appName = "";
                try { const t = document.querySelector(".card-title, h1, h2"); appName = t ? normText(t) : ""; } catch (_) {}
                const ok = window.confirm(
                    "【LINUX DO 助手】检测到未知应用请求授权：\n\n" +
                    (appName ? "应用：" + appName + "\n" : "") +
                    "client_id：" + id + "\n\n" +
                    "点「确定」= 自动点允许，并永久记住这个应用（以后自动授权）。\n" +
                    "点「取消」= 什么都不做，你自己决定。"
                );
                if (!ok) return;
                learnClient(id);
                try {
                    const p = gmJsonGet(OAUTH_PENDING_KEY, []);
                    if (Array.isArray(p)) gmSet(OAUTH_PENDING_KEY, JSON.stringify(p.filter(function (x) { return x !== id; })));
                } catch (_) {}
                doClickAllow();
            };
            ...
            if (clientAllowed(cid)) {
                ...
                if (inBackground) {
                    // 后台不弹窗（会被浏览器自动取消），记录待确认，切到前台再问
                    try {
                        const p = gmJsonGet(OAUTH_PENDING_KEY, []);
                        const arr = Array.isArray(p) ? p : [];
                        if (arr.indexOf(cid) < 0) { arr.push(cid); gmSet(OAUTH_PENDING_KEY, JSON.stringify(arr)); }
                    } catch (_) {}
                    console.warn("[LDH] 后台标签遇到未知 client_id，已记录待前台确认：" + cid);
                    document.addEventListener("visibilitychange", function onVis() {
                        if (document.visibilityState !== "visible") return;
                        document.removeEventListener("visibilitychange", onVis);
                        askAndMaybeAllow(cid);
                    });
                } else {
                    askAndMaybeAllow(cid);
                }
    ...
    const MODES = {
        daily: { key: "daily", name: "日常维护", short: "维护", color: "#2f6f3e", minutes: CFG.DAILY_MINUTES, topics: CFG.DAILY_TOPICS, replies: CFG.DAILY_REPLIES, likes: CFG.DAILY_LIKES, minPosts: 5, safety: 160,      noLimit: false },
        fast:  { key: "fast",  name: "快速升级", short: "升级", color: "#33507a", minutes: CFG.FAST_MINUTES,  topics: CFG.FAST_TOPICS,  replies: CFG.FAST_REPLIES,  likes: CFG.FAST_LIKES,  minPosts: 5, safety: 175,      noLimit: false },
        idle:  { key: "idle",  name: "日常挂机", short: "挂机", color: "#6a4b8a", minutes: CFG.IDLE_MINUTES,  topics: CFG.IDLE_TOPICS,  replies: CFG.IDLE_REPLIES,  likes: CFG.IDLE_LIKES,  minPosts: 8,  safety: Infinity, noLimit: true,  fullRandom: true  }
    };
    ...
    function runArCheckin(force) {
        ...
        arCheckin(function (s) { arText = s; render(); }).then(function (r) {
            ...
            arState = "ok";
            // E1/E3：签到成功 / 今日已签到
            arText = (r && r.checkedIn) ? "签到成功" : "今日已签到";
    ...
    // Q8/Q17：TL0/1 红，TL2/3/4 绿，TL? 灰
    function tlBadge() {
        const tl = me.trustLevel;
        if (tl === null) return '<span style="color:#aaa;"> TL?</span>';
        const col = tl <= 1 ? "#ff8a8a" : "#8fe0b0";
        return '<span style="color:' + col + ';"> TL' + tl + "</span>";
    }
    // B2/B4/Q3/Q8：LDC 显示
    function ldcSpan() {
        if (!me.username) return "";
        ...
        if (ldc.state === "ok") {
            ...
            return base + 'color:#8fe0b0;" title="' + esc(ldc.msg || "可用 LINUX DO Credits，点击刷新") + '">' + esc(ldc.value) + "LDC" + tag + "</span>";
        }
        if (ldc.state === "loading") return base + 'color:#e0c060;" title="' + esc(ldc.msg || "读取中") + '">LDC…</span>';
        if (ldc.state === "mismatch") return base + 'color:#ff8a8a;" title="' + esc(ldc.msg) + '">LDC：账号不符</span>';
        if (ldc.state === "fail") return base + 'color:#ff8a8a;" title="' + esc(ldc.msg || "读取失败，点击重试") + '">LDC：失败</span>';
        return base + 'color:#888;" title="点击读取 Credit 积分">LDC …</span>';
    }
    /* r4 进度文案（Q2=a）
     * 实测(265px 面板、9px 字号、可用 249px)：
     *   原文案 "日常挂机中：主题 888/500 丨 回复 8888/5000 丨 点赞 10/10" = 252px，已溢出 3px；
     *   再挂上 "·本窗口达上限" 会到 311px，溢出 62px。
     * 所以常态就用紧凑文案(219~246px)，任何结束语都放得下，不会被截断。 */
    function progressText() {
        ...
        const M = MODES[activeMode];
        const w = running ? (M ? M.short : "运行") : (finishedOnce ? "结束" : "就绪");
        return w + " " + sent.topics + goal + "题 " + sent.replies + goalR + "帖 " + sent.likes + goalL + "赞";
    }
    function render() {
        const r1 = document.getElementById("ldh_r1"); if (!r1) return;
        const M = MODES[activeMode];
        const timer = running && M ? "⏱ " + mmss(Math.min(elapsed(), totalMs(M))) : frozenTimer;
        r1.innerHTML = esc(me.username || "未登录") + tlBadge() + ldcSpan() +
            '<span style="float:right;color:#8fe0b0;">' + timer + "</span>";
        ...
        // Q8：r4 优先级 —— 刷帖进度 > banMsg > Agent签到 + any封禁
        const r4 = document.getElementById("ldh_r4");
        if (running || finishedOnce) {
            const note = endNote ? ' <span style="color:#ff8a8a;">·' + esc(endNote) + "</span>" : "";
    ...
    function createUI() {
        ...
        p.innerHTML =
            ...
            '<span id="ldh_sync" style="cursor:pointer;font-size:9px;color:#8fe0b0;margin-left:6px;" title="左键同步 / 右键清空已记住的 OAuth 应用">⟳同步</span>' +
        ...
        const syncEl = document.getElementById("ldh_sync");
        syncEl.addEventListener("click", function () { sync(); });
        // Q6=a：右键清空已学习的 OAuth 白名单
        syncEl.addEventListener("contextmenu", function (e) {
            e.preventDefault();
            const n = learnedClients().length;
            if (!n) { alert("没有已记住的 OAuth 应用（3 个内置的不受影响）。"); return; }
            if (confirm("清空已记住的 " + n + " 个 OAuth 应用？\n清空后再遇到它们会重新询问。\n（AgentRouter / AnyRouter / Credit 三个内置的不受影响）")) {
                clearLearnedClients(); alert("已清空。");
            }
        });
